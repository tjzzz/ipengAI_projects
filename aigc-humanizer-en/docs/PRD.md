...
journey
    ...
    section 初次使用
      ...
      免费检测 500 词: 4: Lily
    section 看到结果
      立即看到 AI 率 68% 的检测报告: 5: Lily
      段落分析和修改建议全部可见: 4: Lily
    section 主动改写
      点击「登录后改写」: 3: Lily
      注册/登录: 2: Lily
      扫码支付 ¥10.4: 3: Lily
    section 看到效果
      AI 率降至 22%, 降幅 46pp: 5: Lily
      下载改后 DOCX: 4: Lily
    section 复购与传播
      一周后第二批论文: 4: Lily
      免费重改写 (7天内): 5: Lily
...
- **验收标准**:
  ...
  - [ ] 免费额度 ≤500 词
  - [ ] 检测完成后**先展示完整结果**（AI率+子评分+修改建议），不要求登录
  - [ ] 返回 AI 概率百分比 + 5 维子评分
  - [ ] 改写按钮在结果区引导登录/付费，不打断结果浏览
...
| F3 | 支付宝支付 | P0 | 二维码支付，支持预创建+即时展示；弹窗布局：左栏统计信息（字数/费用/保障），右栏二维码+轮询状态 |
| F4 | 文件上传 | P0 | 支持 TXT、DOCX、PDF 解析 |
| F5 | 文件下载 | P1 | 支持 DOCX、PDF 导出 |
| F6 | 用户管理 | P1 | 注册、登录、订单历史、7 天免费重改写 |
| **F7** | **渐进式交互** | **P1** | **先展示结果 → 后引导登录/付费，不打断用户浏览完整检测报告** |
